...
**Status:** Owner responses reviewed — follow-up decisions pending
...
The owner responses resolve much of the scope. Before updating the requirements source of truth, close the remaining decisions: C-01 (build-time WAV architecture), C-02 (default theme/fallback), C-04 (completion-versus-error priority interpretation), C-07 (strict `/quit` limitation), C-09 (command and mute/re-enable behavior), and C-10 (platform matrix).

Then update [Extension_PRD_Planning.md](Extension_PRD_Planning.md) in this order:

1. **Promote the approved MVP scope**
   - Define a published npm Pi package with global, persisted configuration; all in-scope cues enabled by default; no telemetry; no custom sound paths; and a TUI/local-host-only policy.
   - Record that generic UI errors and `/skill`, `/extension`, and `subagent` cues are out of scope for MVP.

2. **Define the audio-patch pipeline**
   - Specify the approved `@web-kits/audio` integration: pin curated patch definitions, render them to WAVs at package build time, bundle the generated assets, and map extension events to patch sound names.
   - Add a required build-time theme-completeness validation and the approved default/fallback policy.
   - Include dependency versioning and MIT-license attribution requirements for the patch source.

3. **Replace the event matrix with the approved set**
   - Include Pi startup; agent start; tool failure; best-effort Esc-triggered abort; final agent settlement/completion; the selected shutdown policy; and the custom Settings UI interaction sounds.
   - For each, define the Pi hook, sound key, enabled default, reliability classification, debounce group, and priority.
   - Define the provisional priority rule: tool errors have a one-second debounce; completion wins only over pending/colliding lower-priority cues, subject to C-04 confirmation.

4. **Define the Settings UI and configuration schema**
   - Specify the approved root menu: Turn all sounds on, Turn all sounds off, Edit individual sound playback, and Select Audio Theme.
   - Define the individual-event toggle list, keyboard behavior, global config-file location/schema/versioning, and the approved mute/re-enable behavior.
   - State that this is a custom extension UI and not an integration with Pi’s built-in `/settings` UI.

5. **Define the cross-platform contract**
   - If C-10 is approved, declare macOS, native Windows 10/11, and Ubuntu/Debian Linux with PulseAudio/PipeWire or ALSA fallback as MVP targets; declare WSL best-effort/unsupported.
   - Specify each player adapter, its missing-player behavior, non-blocking process handling, output suppression, and session-shutdown cleanup.

6. **Revise acceptance criteria and the release gate**
   - Add package installation/update tests, patch-to-WAV build tests, theme-completeness tests, per-platform playback tests, persisted-settings tests, all approved event-cue tests, one-second debounce/priority tests, and abort-detection tests.
   - Require no playback in JSON/print/RPC or remote-host scenarios under the selected local-host policy.

7. **Keep unresolved decisions explicit**
   - Do not convert the remaining C-01/C-02/C-04/C-07/C-09/C-10 choices into implied requirements. They remain implementation blockers until confirmed.
