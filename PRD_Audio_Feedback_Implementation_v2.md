# Pi-Audio-Feedback — Implementation Specification v2

**Status:** Draft for owner implementation-readiness review  
**Canonical status:** This document supersedes `PRD_Audio_Feedback_Implementation.md` only after owner approval.  
...
```json
{
  ...
  "themes": {
    "core": { "source": "RELEASE-INPUT-REQUIRED", "sha256": "RELEASE-INPUT-REQUIRED" },
    "retro": { "source": "RELEASE-INPUT-REQUIRED", "sha256": "RELEASE-INPUT-REQUIRED" },
    "organic": { "source": "RELEASE-INPUT-REQUIRED", "sha256": "RELEASE-INPUT-REQUIRED" },
    "soft": { "source": "RELEASE-INPUT-REQUIRED", "sha256": "RELEASE-INPUT-REQUIRED" }
  }
}
```

Before implementation/release, the owner or release maintainer must replace every `RELEASE-INPUT-REQUIRED` value with an immutable source URL or repository/ref plus the SHA-256 of the exact patch JSON used. The input patch JSON files are committed under `assets/patches/<theme>.json`; generated modules in `lib/.web-kits` are not authoritative inputs.
...
Please review this v2 specification before implementation. Approval or edits are required for:

1. **Asset provenance:** Fill the four immutable patch source and SHA-256 entries in `assets/patches/manifest.json`.
2. **License:** Approve MIT for package code/generated WAVs or supply another license.
3. **Abort scope:** Approve the experimental, literal-Escape-only heuristic and its intentional false-negative behavior.
4. **Mode policy:** Approve the exact CI and SSH suppression variable lists.
5. **Scheduler:** Approve the single-pending-slot queue, 2-second expiry, watchdog, and coalesced one-second tool-error groups.
6. **Settings behavior:** Approve Space-to-preview and Enter-to-save as separate one-cue actions.
7. **Compatibility:** Approve the Pi/Node ranges and platform release matrix.
8. **Config policy:** Approve first-write timing, validation/migration behavior, atomic write procedure, and last-writer-wins concurrency policy.
